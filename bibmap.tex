...
1. 首先加载bibmap宏包的tex文件第一次编译后，bibmap宏包会将一些设置信息，比如样式文件、bib文件、文献引用等信息写入aux文件。
...
本宏包最初的工作，只是想为修改bib文件方便，而设计一个python脚本，而不去使用bib管理软件，或者使用biber程序(biber使用tex的输入，为修改bib而写一个tex文件有点多余)。做到一定程度后，本来想为biblatex-check脚本做个pr了事，但没想到又接着写下去了，慢慢变成了两个主要功能，一是参考文献bib文件数据修改，二是参考文献格式化，既便于bib文件的修改、转换和保存，也便于格式化后直接给latex应用或者文本复制和网页显示。第二个功能整体上已经类似于bibtex，所以也就继续完善下来，最终形成了目前的bibmap宏包。基于传统的参考文献生成方法，利用natbib等现成宏包，替代bibtex程序，形成一个便于参考文献生成的脚本程序和宏包。
...
因为bibmap宏包的标注样式基于natbib宏包实现，而要使用分章的参考文献，那么需要使用chapterbib宏包，因此多这两个宏包做一些介绍方便使用。
