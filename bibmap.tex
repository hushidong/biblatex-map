...
\begin{description}
  ...
  \item[bibtable] 用于确定是否采用表格形式的文献表。值为true或false，当给出为true时生成表格形式的文献表。默认的表格形式是两栏的表格，第一列是文献的序号，第二列是文献的完成著录信息。如果要修改表格的形式，可在设置bibtable=true后对thebibliography环境进行重定义，该环境的默认定义为：
      ...
      如果要修改表格中输出的内容形式，比如3栏或者序号变成为姓名等，则需要修改bibmap.py 程序。在函数printbibliography 中，由bibtable=true得到bibliotableflag标签为true，因此需修改其判断为true后的代码。序号内容的修改则需要修改bibmapciteb相关的输出信息。
